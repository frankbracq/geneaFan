...
export function showGedcomFilesModal(files, userInfo) {
  ...
  function initializeTooltips(modalDiv) {
    const tooltipTriggerList = modalDiv.querySelectorAll(
  ...
  async function handleActionClick(e) {
    ...
    if (actionIcon) {
      ...
      console.log(
        `Action: ${action}, File ID: ${fileId}, Data Link: ${dataLink}`
      );
      ...
      try {
        ...
        console.error(
          `Error handling action ${action} for file ID ${fileId}:`,
          error
        );
      }
    }
  ...
  async function initializeShareForm(sanitizedFileId) {
    ...
    if (shareForm.dataset.initialized) {
      console.log(
        `The share form for file ID ${sanitizedFileId} is already initialized.`
      );
      return;
    }
    ...
    if (!emailInputs.length) {
      console.error(
        `No email fields found for form ID: shareForm-${sanitizedFileId}`
      );
      return;
    }
    ...
    let noValidEmailError = errorContainer.querySelector(
    ...
    reaction(
      ...
      (isValid) => {
        ...
        const shareSubmitButton = document.getElementById(
          `shareSubmit-${sanitizedFileId}`
        );
  ...
  function showConfirmationMessage(
    ...
    message.textContent =
      "Confirmez-vous le partage du fichier avec les adresses suivantes :";
  ...
  function showSuccessMessage(sanitizedFileId, result) {
    ...
    let messageText =
      "Le fichier a été partagé avec succès avec les adresses suivantes :<ul>";
    ...
    const shareFormContainer = document.getElementById(
      `shareForm-${sanitizedFileId}`
    ).parentNode;
  ...
  function showButtonSpinner(sanitizedFileId) {
    const spinner = document.getElementById(
      `shareButtonSpinner-${sanitizedFileId}`
    );
    if (spinner) {
