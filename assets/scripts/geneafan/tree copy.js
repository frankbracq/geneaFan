...
function initializeFamilyTree() {
    ...
    initializing = true; // Début de l'initialisation
    ...
    const initialRootId = config.root; // Assumed to be defined
    ...
    // Use onInit to dynamically set the root = oldest ancestor of the initial root
    // onInit is called only once when the nodes have been created and loaded to the DOM.
    /*
    family.onInit(() => {
        console.log('Family tree onInit');
        const rootNode = family.getNode(initialRootId);
        if (rootNode) {
            const rootId = getOldestAncestorOf(rootNode.id, "mother"); // or "mother" to prioritize mother
    ...
    */
    ...
    initializing = false; // Fin de l'initialisation
}
...
reaction(
    ...
    (newRootId) => {
        console.log("New root ID detected in reaction:", newRootId);
        if (!initializing) {
            if (!family) {
                initializeFamilyTree();
            }
            const rootId = getOldestAncestorOf(newRootId, "both"); // or "mother" or "father" to prioritize mother or father
